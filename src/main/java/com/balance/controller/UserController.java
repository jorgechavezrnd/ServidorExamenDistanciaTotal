...
public class UserController {
    ...
    public String getDistanciaRecorrida(Model model) {
        ...
        while(iterator.hasNext()){
            LocationHistory aux = iterator.next();
            if(aux.getUser().equals(user.getId())) {
                locationHistoryList.add(aux);
            }
        }

        Collections.sort(locationHistoryList, new Comparator<LocationHistory>() {
            @Override
        ...
        Iterator<LocationHistory> iterator1 = locationHistoryList.iterator();

        Double distanciaTotal = 0.0;

        while (iterator1.hasNext()) {

            LocationHistory aux = iterator1.next();

            if (iterator1.hasNext()) {
                LocationHistory aux2 = iterator1.next();

                Double x1 = new Double(aux.getLatitude());
                Double x2 = new Double(aux2.getLatitude());
                Double y1 = new Double(aux.getLongitude());
                Double y2 = new Double(aux2.getLongitude());
